-- Guest self-service actions (step out, step back in, leave), mirroring
-- what a signed-in app player can already do directly via RLS
-- ("players update own row"). Guests have no auth.uid(), so these go
-- through the same session-token pattern as guest_join_game/guest_status.
...
-- Extend guest_status with the player's most recently confirmed match, so
-- the web guest can see whether they won or lost, same as the app.
