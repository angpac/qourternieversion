-- Bugfix: v_status was declared `text`, and assigning a typed text
-- variable (not a literal) into game_players.status (type player_status)
-- requires an explicit cast — Postgres only auto-coerces untyped string
-- literals, not variables. This broke every join once approval support
-- was added. Declaring v_status as player_status directly fixes it.
